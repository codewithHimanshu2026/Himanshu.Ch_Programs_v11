...
TOTAL_TEST_TIME = 300  # 5 Minutes

# Color Palette (Terminal Developer Matrix Theme)
COLOR_BG = "#0d1117"
COLOR_CARD = "#161b22"
COLOR_TEXT = "#58a6ff"
COLOR_ACCENT = "#00ff66"  # Matrix Terminal Green
COLOR_CYAN = "#38d5ff"
COLOR_MUTED = "#30363d"
COLOR_PRIMARY = "#bc8cff"
...
class MajorQuizEngineApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("ONLINE QUIZ PORTAL - MAJOR PROJECT")
        ...
        self.user_domain = tk.StringVar(value="Full Stack Developer")
...
# -----------------------------
# PAGE 1: WELCOME & ANIMATED DEVELOPER HUB
# -----------------------------
class WelcomePage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # Main Card Overlay
        card = tk.Frame(self, bg=COLOR_CARD, bd=2, relief="groove")
        card.place(relx=0.5, rely=0.5, anchor="center", width=580, height=560)

        # Top Animated Cyber Header
        self.header_canvas = tk.Canvas(
            card, width=540, height=110, bg="#0d1117", highlightthickness=0
        )
        self.header_canvas.pack(pady=15)

        self.header_canvas.create_rectangle(
            5, 5, 535, 105, outline=COLOR_ACCENT, width=2
        )

        self.glow_title = self.header_canvas.create_text(
            270,
            25,
        ...
        self.header_canvas.create_text(
            270,
            70,
            text="[SYSTEM_STATUS: ONLINE // AUTH_REQUIRED]",
        ...
        self.animate_text_glow()

    def animate_text_glow(self):
        """Neon Glow Pulse Animation."""
        color = COLOR_ACCENT if self.pulse_state else COLOR_CYAN
        self.header_canvas.itemconfig(self.glow_title, fill=color)
        self.pulse_state = not self.pulse_state
        self.after(750, self.animate_text_glow)
    ...
    def validate_and_proceed(self):
        if (
            ...
            messagebox.showwarning(
                "Access Denied", "Please enter Developer ID and Access Code!"
            )
            return
...
class InstructionsPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(
            card,
            text="SYSTEM INSTRUCTIONS & RULES",
        ...
        rules_text = (
            "1. ASSESSMENT FORMAT:\n"
            "   • Total Questions: 10 Technical MCQs.\n"
            "   • Time Limit: 05:00 Minutes (300 Seconds).\n\n"
            "2. SCORING METHOD:\n"
            "   • Correct Execution: +1 Point.\n"
            "   • Incorrect / Skipped: 0 Points.\n\n"
            "3. EVALUATION BEHAVIOR:\n"
            "   • NO live feedback or red/green validation during the test.\n"
            "   • Full detailed audit report will be generated after submission.\n\n"
            "4. NAVIGATION:\n"
            "   • Use Question Palette to jump directly across questions.\n"
...
class TestPage(tk.Frame):
    ...
    def update_palette_colors(self):
        for i in range(len(self.controller.questions)):
            ...
            if status == "answered":
                btn.config(bg=COLOR_SUCCESS, fg="#0d1117")
            elif status == "review":
                btn.config(bg=COLOR_WARNING, fg="#0d1117")
            else:
                btn.config(bg="#0d1117", fg="white")
...
class ScorecardPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.card.place(
            relx=0.5, rely=0.5, anchor="center", width=720, height=620
        )
    ...
    def on_show(self):
        ...
        if pct == 100:
            rank = "PRINCIPAL ARCHITECT ⚡ (GRADE S)"
        elif pct >= 80:
            rank = "SENIOR DEVELOPER 💻 (GRADE A)"
        elif pct >= 50:
            rank = "ASSOCIATE DEV 🛠 (GRADE B)"
        else:
            rank = "JUNIOR CODER 🐒 (GRADE C)"

        header_text = (
            f"Developer: {self.controller.user_name.get()} | Code: {self.controller.user_roll.get()}\n"
        ...
        for i, q in enumerate(self.controller.questions, start=1):
            user_ans = (
                self.controller.user_responses[i - 1]["selected"]
                or "Not Answered"
            )
